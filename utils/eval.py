...
class Evaluator:
    def __init__(
        self,
        metrics_at_k: Dict[str, List[int]] = {
            "recall": [100, 200, 500],
            "map": [100],
            "mrr": [100],
        },
    ...
    def compute_all_metrics(
        self, all_results: List[List[int]], all_ground_truths: List[List[int]]
    ):
        scores = dict()
        for k in self.metrics_at_k["recall"]:
            recall_scalar = self.compute_mean_score(
                self.recall, all_ground_truths, all_results, k
            )
            scores[f"recall@{k}"] = recall_scalar

        for k in self.metrics_at_k["map"]:
            map_scalar = self.compute_mean_score(
                self.average_precision, all_ground_truths, all_results, k
            )
            scores[f"map@{k}"] = map_scalar

        for k in self.metrics_at_k["mrr"]:
            mrr_scalar = self.compute_mean_score(
                self.reciprocal_rank, all_ground_truths, all_results, k
            )
    ...
    def compute_mean_score(
        ...
        return mean(
            [
                func(truths, res, k)
                for truths, res in zip(all_ground_truths, all_results)
            ]
        )
    ...
    def reciprocal_rank(
        self, ground_truths: List[int], results: List[int], k: int = None
    ):
        k = len(results) if k is None else k
        return max(
            [
                1 / (i + 1) if d in ground_truths else 0.0
    ...
    def average_precision(
        ...
        k = len(results) if k is None else k
        p_k = [
            self.precision(ground_truths, results, k=i + 1) if d in ground_truths else 0
            for i, d in enumerate(results[:k])
        ]
...
class BiEncoderEvaluator(Evaluator):
    ...
    def __call__(
        ...
        # Encode queries.
        q_embeddings = model.q_encoder.encode(
            texts=self.queries, device=device, batch_size=batch_size
        )
        d_embeddings = model.d_encoder.encode(
            texts=self.documents, device=device, batch_size=batch_size
        )
        ...
        all_results = [
            [result["corpus_id"] + 1 for result in results] for results in all_results
        ]  # Extract the doc_id only -> List[List[int]] (NB: +1 because article ids start at 1 while semantic_search returns indices in the given list).
