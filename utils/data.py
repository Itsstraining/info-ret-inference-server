...
class TextPreprocessor:
    ...
    def preprocess(
        ...
        return series.progress_apply(
            lambda text: self.preprocess_text(
                text, lowercase, remove_punct, remove_num, remove_stop, lemmatize
            )
        )

    def preprocess_text(
        self, text, lowercase, remove_punct, remove_num, remove_stop, lemmatize
    ):
        if lowercase:
    ...
    def _remove_numbers(self, doc):
        return [
            t
            for t in doc
            if not (t.is_digit or t.like_num or re.match(".*\d+", t.text))
        ]
...
class BSARDataset(Dataset):
    def __init__(self, queries: pd.DataFrame, documents: pd.DataFrame):
        ...
        self.one_to_one_pairs = self.get_one_to_one_relevant_pairs(
            queries
        )  # qid -> rel_docid_i
        self.one_to_many_pairs = self.get_one_to_many_relevant_pairs(
            queries
        )  # qid -> {rel_docid_1, ..., rel_docid_n}
    ...
    def get_one_to_many_relevant_pairs(
        self, queries: pd.DataFrame
    ) -> Dict[str, Set[str]]:
        return (
            queries.set_index("id")["article_ids"]
            .str.split(",")
            .apply(lambda x: [int(i) for i in x])
            .to_dict()
        )

    def get_one_to_one_relevant_pairs(
        self, queries: pd.DataFrame
    ) -> List[Tuple[int, int]]:
        data = (
            queries.assign(article_ids=lambda d: d["article_ids"].str.split(","))
            .set_index(queries.columns.difference(["article_ids"]).tolist())[
                "article_ids"
            ]
            .apply(pd.Series)
            .stack()
            .reset_index()
            .drop(
                ["category", "subcategory", "extra_description", "question", "level_5"],
                axis=1,
            )
