...
class BiEncoder(nn.Module):
    def __init__(
        ...
        d_model_name_or_path: Optional[
            str
        ] = None,  # model name (Hugging Face) or path for the document encoder.
        truncation: bool = False,  # whether to truncate the inputs to a length of 'chunk_size'.
        max_input_len: Optional[
            int
        ] = None,  # size of the maximum sequence size when truncation is set to True.
        chunk_size: Optional[
            int
        ] = None,  # size of the truncated input if 'truncation' is True OR size of the split chunks if 'truncation' set to False.
        ...
        self.scale = scale
        self.q_encoder = PassageEncoder(
            model_name_or_path=q_model_name_or_path,
        ...
        if is_siamese:
            ...
            d_model_name_or_path = (
                d_model_name_or_path
                if d_model_name_or_path is not None
                else q_model_name_or_path
            )
            self.d_encoder = PassageEncoder(
                model_name_or_path=d_model_name_or_path,
                truncation=truncation,
                max_input_len=max_input_len,
                chunk_size=chunk_size,
                window_size=window_size,
    ...
    def forward(
        self, q_input_ids: T, q_attention_masks: T, d_input_ids: T, d_attention_masks: T
    ) -> Tuple[T, T]:
    ...
    def collate_batch(self, batch: List[Tuple[str, str]]) -> Tuple[T, T, T, T]:
        ...
        """
        queries, documents = map(
            list, zip(*batch)
        )  # Unzip a list of tuples into individual lists: [query, query, ...], [document, document, ...]
        q_input_ids, q_attention_masks = self.q_encoder.encoder.word_encoder.tokenize(
            queries
        )
        d_input_ids, d_attention_masks = self.d_encoder.encoder.word_encoder.tokenize(
            documents
        )
        return {
            "q_input_ids": q_input_ids,
            "q_attention_masks": q_attention_masks,
            "d_input_ids": d_input_ids,
            "d_attention_masks": d_attention_masks,
        }
    ...
    def save(self, output_path: str):
        ...
        with open(
            os.path.join(output_path, f"{type(self).__name__}_config.json"), "w"
        ) as fOut:
    ...
    def load(input_path: str):
        ...
        if config["is_siamese"] == True:
            return BiEncoder(
                q_model_name_or_path=os.path.join(
                    input_path, "p_encoder", "WordEncoder"
                ),
                **config,
            )
        else:
            return BiEncoder(
                q_model_name_or_path=os.path.join(
                    input_path, "q_encoder", "WordEncoder"
                ),
                d_model_name_or_path=os.path.join(
                    input_path, "d_encoder", "WordEncoder"
                ),
                **config,
            )
...
class PassageEncoder(nn.Module):
    ...
    def save(self, output_path: str):
        ...
        torch.save(
            self.state_dict(),
            os.path.join(output_path, f"{type(self).__name__}_model_state.bin"),
        )
    ...
    def load(input_path: str):
        ...
        return PassageEncoder(
            model_name_or_path=os.path.join(input_path, "WordEncoder"), **config
        )
...
class WordEncoder(nn.Module):
    ...
    def tokenize(self, texts: List[str]) -> Tuple[T, T]:
        ...
        tokenized = self.tokenizer(
            ...
            return_tensors="pt",
        )  # Truncation to 'max_length' AND padding to max sequence in batch (see https://huggingface.co/transformers/preprocessing.html)
        input_ids, attention_masks = self._transform_inputs(
    ...
    def _transform_inputs(self, input_ids: T, attention_masks: T) -> Tuple[T, T]:
        ...
        if batch_max_seq_len <= self.chunk_size:
            ...
            attention_masks = attention_masks[
                :, 1:
            ]  # T[batch_size x batch_max_seq_len-1]
            batch_max_seq_len -= 1
            ...
            num_windows = math.floor(
                (batch_max_seq_len - chunk_size) / (chunk_size - self.window_size)
            )
            num_repeated_tokens = num_windows * self.window_size
            batch_seq_len = (
                math.ceil((batch_max_seq_len + num_repeated_tokens) / chunk_size)
                * chunk_size
            )
            input_ids = torch.nn.functional.pad(
                input=input_ids,
                pad=(0, batch_seq_len - batch_max_seq_len),
                mode="constant",
                value=self.tokenizer.pad_token_id,
            )
            attention_masks = torch.nn.functional.pad(
                input=attention_masks,
                pad=(0, batch_seq_len - batch_max_seq_len),
                mode="constant",
                value=0,
            )
            ...
            input_ids = input_ids.unfold(
                dimension=1, size=chunk_size, step=chunk_size - self.window_size
            ).permute(0, 2, 1)
            attention_masks = attention_masks.unfold(
                dimension=1, size=chunk_size, step=chunk_size - self.window_size
            ).permute(0, 2, 1)
            splits_size = input_ids.size(2)

            # Add CLS token ids (with attention masks of 1) in the beginning of all split sequences.
            cls_input_ids = torch.full(
                (batch_size, 1, splits_size), self.tokenizer.cls_token_id
            )
            ...
            cls_attention_masks[
                input_ids[:, 1, :].unsqueeze(1) != self.tokenizer.pad_token_id
            ] = 1
        ...
        if self.pooling_mode == "cls":
            ...
            return self.pool(
                token_embeddings=token_embeddings[
                    :, 0, :, :
                ],  # get all CLS token embeddings in the batch.
                attention_masks=attention_masks[
                    :, 0, :
                ],  # get the attention masks of all CLS tokens in the batch.
                pooling_mode="mean",
            )
        else:
            ...
            return self.pool(
                ...
                ),  # exclude all CLS token embeddings and reshape the batch tensor into a 3D matrice of size [batch_size x batch_max_seq_len x dim].
                attention_masks=attention_masks[:, 1:, :]
                .permute(0, 2, 1)
                .reshape(
                    batch_size, -1
                ),  # exclude attention masks corresponding to the CLS token embeddings and reshape the batch tensor into a 2D matrice of size [batch_size x batch_max_seq_len].
                pooling_mode=self.pooling_mode,
            )

    def pool(
        self, token_embeddings: T, attention_masks: T, pooling_mode: str = "mean"
    ) -> T:
        """
        Args:
        ...
        if pooling_mode == "max":
            ...
            attention_masks_expanded = (
                attention_masks.unsqueeze(-1).expand(token_embeddings.size()).float()
            )
            ...
            sum_mask = attention_masks_expanded.sum(
                dim=1
            )  # -> Tensor[batch_size, embedding_dim] where each value is the length of the corresponding passage.
            sum_mask = torch.clamp(
                sum_mask, min=1e-9
            )  # Make sure not to have zeros by lower bounding all elements to 1e-9.
            passage_vectors = (
                sum_embeddings / sum_mask
            )  # Divide each dimension by the sequence length.
